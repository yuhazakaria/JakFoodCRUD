package com.example.jakfoodcrud;
...
import com.example.jakfoodcrud.adapter.FoodAdapter;
import com.example.jakfoodcrud.helper.SessionManager;
import com.example.jakfoodcrud.model.DataKategoriItem;
...
public class MainActivity extends SessionManager implements FoodAdapter.OnFoodClick, SwipeRefreshLayout.OnRefreshListener{
    ...
    private static final int RED_FILE_CODE = 1;
            ...
            public void onResponse(Call<ResponseMakanan> call, Response<ResponseMakanan> response) {
                if (response.isSuccessful()){
                    ...
                    for (int index = 0; index<dataMakanan.size(); index++){
                        ...
                        name[index] = dataMakanan.get(index).getIdMakanan();
                        time[index] = dataMakanan.get(index).getIdMakanan();
                        images[index] = dataMakanan.get(index).getIdMakanan();
                        strIdUser = id[index];
                    }
    ...
    private void showFileChooser() {
        ...
        startActivityForResult(Intent.createChooser(intentGallery, "Select picture"), MainActivity.RED_FILE_CODE);
    }
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if (requestCode == RED_FILE_CODE && resultCode == RESULT_OK
    ...
    private void fetchUpdate() {
        ...
        if (TextUtils.isEmpty(namaMakanan)) {
            edtUpdateNameFood.setError("Nama makanan harus nberbeda");
        } else if (ivPreviewInsert.getDrawable() == null) {
